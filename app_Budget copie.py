...
from PySide6.QtWidgets import QMainWindow, QWidget, QVBoxLayout, QPushButton, QLabel, QStackedWidget, QStackedLayout
...
class Budget_Widget(QWidget):
    def __init__(self, parent, connecteur_sql, fenetre_principale):
        ...
        self.listeUtilisateurs = self.lireTousLesUtilisateurs()
        ...
        self.layoutBouton = QGridLayout()
        self.gridBudget1 = QGridLayout()
        self.gridBudget2 = QGridLayout()

        self.stack1 = QWidget()
        self.stack2 = QWidget()

        utilisateur = self.utilisateur_budget
        self.stack1_UI()
        self.stack2_UI()
        self.utilisateur_budget = utilisateur
        ...
        danyOuDiane = 1
        if g_UtilisateurBudget == 'Diane':
            danyOuDiane = 2
        self.Stack.setCurrentIndex(danyOuDiane)
        ...
        gridSommaire = QVBoxLayout()
        gridSommaire.addWidget(QLabel('Dany'), 0, Qt.AlignmentFlag.AlignTop)
        gridSommaire.addWidget(QLabel('1'))
        gridSommaire.addWidget(QLabel('2'))
        gridSommaire.addWidget(QLabel('3'))
        gridSommaire.addWidget(QLabel('4'))
        gridSommaire.addWidget(QLabel('5'))
        gridSommaire.addWidget(QLabel('6'))

        # Pour être certain que les boutons Sauvegarder et Quitter s'affiche à droite de la fenêtre
        self.layoutBouton = QGridLayout()
        self.layoutBouton.addWidget(self.boutonStat, 0, 0)
        self.layoutBouton.addWidget(QLabel(), 0, 1)
        self.layoutBouton.addWidget(QLabel(), 0, 2)
        self.layoutBouton.addWidget(self.labelMessage, 0, 3)
        self.layoutBouton.addWidget(self.boutonCache, 0, 4)
        self.layoutBouton.addWidget(self.boutonQuitter, 0, 5)




        self.layoutPrincipal = QVBoxLayout()
        self.layoutPrincipalButton = QHBoxLayout()
        self.layoutPrincipal.addLayout(self.layoutTop)
        self.layoutPrincipal.addWidget(self.Stack)
        self.layoutPrincipalButton = self.layoutBouton
    ...
    def stack1_UI(self):
        self.utilisateur_budget = 'Dany'
        self.afficherBudget()
        self.stack1.setLayout(self.gridBudget1)

    def stack2_UI(self):
        self.utilisateur_budget = 'Diane'
        self.afficherBudget()
        self.stack2.setLayout(self.gridBudget2)
    ...
    def afficherBudget(self):
        global g_CheckBox_ListeSaisieManuelle, g_LineEdit, \
            g_LabelLigneBudget, g_LabelSection
        ...
        if self.utilisateur_budget == 'Dany':
            gridBudget = self.gridBudget1
        else:
            gridBudget = self.gridBudget2

        montantTotalFacturesCommunes = 0

        self.chargerTransactionsFichier(self.listeDatesPeriodes)
        ...
        for section in self.sectionsBudget:
            if section != 'Sommaire':
                ...
                if informations_Budget:
                    ...
                    if ligne == 0:
                        gridBudget.addWidget(g_LabelSection[section], ligne, 0 + noColonne,
                                                          alignment=Qt.AlignmentFlag.AlignTop)
                    else:
                        gridBudget.addWidget(g_LabelSection[section], ligne, 0 + noColonne)
                    gridBudget.addWidget(QLabel("Banque"), ligne, 1 + noColonne)
                    ...
                    for nomTx in listeMontantsPrevus:
                        ...
                        montantTransactions = self.calculerMontant(listeLignesBudget[nomTx])
                        g_LineEdit[listeLignesBudget[nomTx]] = self.LineEdit(objectName=f"{section},"
                        ...
                        gridBudget.addWidget(g_LabelLigneBudget[listeLignesBudget[nomTx]],ligne,
                                                      0 + noColonne)
                        gridBudget.addWidget(g_LineEdit[listeLignesBudget[nomTx]], ligne, 1 + noColonne,
        ...
        # if self.utilisateur_budget == 'Dany':
        #     self.gridBudget1 = gridBudget
        # else:
            # self.gridBudget2 = gridBudget

        if noSection == 0:
            QMessageBox.information(self.fenetre_principale,
                                    "",
                                    f"L'utilisateur {self.utilisateur_budget} n'a aucune donnée pour faire un budget",
                                    )

    def lireTousLesUtilisateurs(self):
        ...
        if g_SQLQuery[g_nbQueryOuverte].exec(requete):
            while g_SQLQuery[g_nbQueryOuverte].next():
                self.utilisateur_budget = g_SQLQuery[g_nbQueryOuverte].value(dernierCodeUtilisateur)

        if not self.utilisateur_budget:
            self.utilisateur_budget = 'Dany'
    ...
    def chercher_ligneBudget_DansTableTransactions(self, nom_tx, date_tx, est_depense, liste_dates_periodes):
        global g_nbQueryOuverte, g_SQLQuery
    ...
    def ajouterTx_DansTableTransactions(self, ligne_budget, no_periode, nom_tx, date_tx, depense_tx, revenu_tx):
        global g_nbQueryOuverte, g_SQLQuery
    ...
    def calculerMontant(self, ligne_budget):
    ...
    class LineEdit(QLineEdit):
        def mouseDoubleClickEvent(self, event):
            global g_InformationsMontantSaisi

            g_InformationsMontantSaisi = self.objectName().split(",")
            ChoisirChargementTxOuAfficherTx()
    ...
    class ChoisirChargementTxOuAfficherTx(QMainWindow):
        def __init__(self):
            ...
            self.Tx = self
            self.windowTx = AfficherTransactionsCellule(g_UtilisateurBudget)

            self.windowTx.setWindowFlags(Qt.WindowStaysOnTopHint)
            self.windowTx.setWindowOpacity(0.85)
            self.windowTx.show()

    def chargerTransactionsFichier(self, liste_dates_periodes):
        import os.path
        from os import path

        fichierUtilisateur = f"tx{self.utilisateur_budget}.xlsx"
        fichierValide = False
        ...
        if path.exists(fichierUtilisateur):
            ...
            for i in df.index:
                ...
                if dateTxString[0:4] == g_AnneeBudget:
                    ...
                    infosLigneBudget = self.chercher_ligneBudget_DansTableTransactions(nomTx, dateTxString,
                                                                                       estDepense, liste_dates_periodes)
                    if not infosLigneBudget[0]:
                        self.ajouterTx_DansTableTransactions(infosLigneBudget[1], infosLigneBudget[2], nomTx, dateTx,
                                                        depenseTx, revenuTx)
        return fichierValide


    def event_afficherTx(self, event):
        global g_InformationsMontantSaisi

        print(event)

        g_InformationsMontantSaisi = event.objectName().split(",")
        print(g_InformationsMontantSaisi)
        ChoisirChargementTxOuAfficherTx()
    ...
    def lireBudget(self, no_section, version_budget):
        ...
        requete = "SELECT " \
                  ...
                  f"WHERE " \
                  f"   type_budget.codeUtilisateur = '{self.utilisateur_budget}' AND " \
        ...
        if not budget_Trouve:
            requete_Insert = "INSERT INTO " \
                             ...
                             f"    ligneBudget, " \
                             f"    {g_AnneeBudget}, " \
                             f"    '{self.utilisateur_budget}', 0," \
                             f"    {g_NoPeriodeBudget} " \
                             "FROM " \
                             "     type_budget " \
                             f"WHERE " \
                             f"    codeUtilisateur = '{self.utilisateur_budget}' AND " \
    ...
    #         self.labelMessage.setText(f"La dernière sauvegarde du budget de {self.utilisateur_budget} s'est bien "
    #                                   f"effectuée")
    ...
    #     self.afficherLignesCacheesBudget = False
    #
    #     self.gridBudgetDany.deleteLater()
    #     self.gridBudgetDiane.deleteLater()
    #     self.layoutPrincipalButton.deleteLater()
    #
    #     self.gridBudgetDany = QGridLayout()
    #     self.gridBudgetDiane = QGridLayout()
    #     self.layoutPrincipalButton = QVBoxLayout()
    #     self.sectionsBudget = lireSectionBudget()

    def onActivated_Utilisateur(self, idx):
        self.utilisateur_budget = self.listeUtilisateurs[idx]

        danyOuDiane = 1
        if self.utilisateur_budget == 'Diane':
            danyOuDiane = 2
        self.Stack.setCurrentIndex(danyOuDiane)
...
class AfficherTransactionsCellule(QMainWindow):
    def __init__(self, utilisateur_budget):
        super().__init__()

        self.readSettingsFenTx()

        self.setWindowTitle(f"Liste des transactions de : {g_InformationsMontantSaisi[3]}")
        self.addAction("Ok")

        self.model = QSqlTableModel(self)
        self.model.setTable("transactions")
        self.model.setFilter(f"ligneBudget = {g_InformationsMontantSaisi[1]} AND anneeBudget = {g_AnneeBudget} "
                             f"AND "
                             f"codeUtilisateur = '{ut}' AND noPeriode = {g_NoPeriodeBudget}")
